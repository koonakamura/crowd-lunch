...
export default function AdminPage() {
  ...
  const [savedMenuItems, setSavedMenuItems] = useState<MenuItemData[]>([])

  const weekdayDates = generateWeekdayDates(new Date(), 5)

  const { data: orders } = useQuery({
    queryKey: ['todayOrders', formatDateForApi(selectedDate)],
    queryFn: () => apiClient.getTodayOrders(formatDateForApi(selectedDate)),
  ...
  const createMenuMutation = useMutation({
    mutationFn: (menuData: { date: string; title: string; photo_url: string }) => 
      apiClient.createMenu(menuData),
    onSuccess: async (menuResponse) => {
      const results = await Promise.allSettled(
        menuItems
          .filter(item => item.name.trim() && item.price > 0)
          .map(item => 
            apiClient.createMenuItem(menuResponse.id, {
              name: item.name,
              price: item.price,
              stock: item.stock
            })
      ...
      if (failedItems.length > 0) {
        console.error('Some menu items failed to create:', failedItems)
      }
